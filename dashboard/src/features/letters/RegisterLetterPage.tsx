...
import RegisterLetterForm from './RegisterLetterForm';

const FORM_ID = 'register-letter-form';

export default function RegisterLetterPage() {
  ...
  // Registering an incoming letter is a Devonxona action. As a modal this page
  // was only reachable from a Devonxona-gated button; as a route anyone could
  // paste the URL, so guard here too (the backend enforces `not-devonxona`
  // regardless — this is the "don't surface controls a role can't use" rule).
  if (!acting.roles.includes('ROLE_DEVONXONA')) {
    return <Navigate to="/letters" replace />;
  }

  const onClose = () => navigate('/letters');
    ...
    <div className="flex min-h-svh flex-col bg-background">
      ...
      <header className="flex h-14 shrink-0 items-center gap-3 border-b border-line bg-surface px-4 md:hidden">
        ...
        <h1 className="truncate text-base font-semibold text-ink">
          {t('dashboard:letters.register.title')}
        </h1>
      </header>
        ...
        <div>
          <Button asChild variant="ghost" size="sm" className="-ml-2 mb-1">
            <Link to="/letters">
              ...
              {t('dashboard:letters.register.back')}
            </Link>
          </Button>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            {t('dashboard:letters.register.description')}
          </p>
        </div>
