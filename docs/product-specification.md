...
- **Employment-order extract ("buyruqdan ko'chirma")** — the certified extract of the hiring order signed by the Director, attached as a required document (PDF, JPG, or PNG) when HR creates the employee profile.
- **Position instructions ("lavozim yo'riqnomasi")** — the document defining the employee's job duties, attached as a required document (PDF, JPG, or PNG) when HR creates the employee profile, alongside the employment-order extract. Both the document author (typically HR) and the employee can reference it. When the position is updated, the version history is preserved.
- **Termination-order extract ("ishdan bo'shatish buyrug'idan ko'chirma")** — terminating an employee requires attaching the certified extract of the termination order signed by the Director. The termination cascade automatically revokes the employee's active ERI certificates.
