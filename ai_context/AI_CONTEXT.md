...
**Status** — Build prompt set complete (17 files, ~7,700 lines in [`docs/dashboard-prompts/`](../docs/dashboard-prompts/)). **Steps 01–08 landed (2026-05-25 → 2026-05-26).**
...
**Build state:** `npm run build` → 2844 modules, 102.66 KB CSS, 660 KB JS / 202 KB gzip. Deps added during the run: `react-hook-form@^7.76` + `@hookform/resolvers@^5.4`. The 500 KB chunk warning is cosmetic — code-splitting is a step-14 concern.

**Next:** `00-master.md` + `09-flow2-employees-list.md` for Flow 2 — xodimlar list with desktop table / mobile card stack + search + unit/status filters + pagination. The shadcn `form` primitive still hasn't been needed (raw `useForm` is enough); reconsider hand-adding when the 4-step employee wizard lands in step 10.

---

## Open questions / known gaps

- **Dashboard implementation in progress** — steps 01–08 are done; step 09 (Flow 2 — xodimlar list with desktop table / mobile card stack + search + unit/status filters + pagination) is next, then sequentially through `15-final-qa.md`. The shadcn `form` primitive is still missing (Nova preset's registry silently skipped it in step 02) but hasn't been needed — the step-08 unit form uses raw `useForm` from react-hook-form. Reconsider hand-adding when the 4-step employee wizard in step 10 needs the `Form` wrapper for nested validation. Out-of-scope items deliberately deferred per master §17: real backend, real PFX parsing, real E-IMZO plugin, real SMS/email OTP, automated tests.
