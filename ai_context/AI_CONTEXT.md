...
**Status** — Build prompt set complete (17 files, ~7,700 lines in [`docs/dashboard-prompts/`](../docs/dashboard-prompts/)). **Steps 01–06 landed (2026-05-25 → 2026-05-26)** — scaffold + Tailwind v4 + shadcn/ui (31 primitives) + react-i18next (UZ canonical, RU/EN stubs) + react-router-dom v6 with `basename: /Devon/dashboard` + Zustand auth store + `RequireAuth` deep-link preserver + mobile-first split-pane login + AppShell with sidebar drawer (Sheet on mobile, persistent 240px column on lg+) + full-width `<main>` (per [`LESSONS.md`](./LESSONS.md)) + **mock-backend foundation** (zod schemas + namespaced localStorage tables + 200–600 ms simulated latency + 3% mock failure on mutations + realistic Uzbek seed: 25 units across 4-level hierarchy / 30 employees with hand-crafted FIOs and matching users / 30 primary assignments / 25 certificates in the 18/4/2/1 status split / ~70 audit entries / 14 positions). Auth store now does real sha256 hash compare against `findUserByEmail` (step 04's literal-credential stopgap is gone). UserMenu's reset-demo calls `resetAndSeed()`. `main.tsx` runs `seedIfEmpty()` before React mounts. `npm run build` clean (94 KB CSS, 471 KB JS / 148 KB gzip). Outstanding from step 02: the shadcn `form` primitive's registry entry is silently skipped by the Nova preset — hand-add when a real form lands in step 10 (employee wizard). Next session begins with `00-master.md` + `07-dashboard-home.md` to wire the home page (stats / activity / quick actions / expiring-cert alert) — the first real consumer of `listEmployees` / `listCertificates` / `listAudit`.

---

## Open questions / known gaps

- **Dashboard implementation in progress** — steps 01–06 are done; step 07 (dashboard home — stats, recent activity, quick actions, expiring-cert alert) is next, then sequentially through `15-final-qa.md`. Hand-add the shadcn `form` primitive when an actual form first appears (Nova preset's registry silently skips it). Out-of-scope items deliberately deferred per master §17: real backend, real PFX parsing, real E-IMZO plugin, real SMS/email OTP, automated tests.
