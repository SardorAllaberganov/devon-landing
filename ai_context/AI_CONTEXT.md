...
**Status** — Build prompt set complete (17 files, ~7,700 lines in [`docs/dashboard-prompts/`](../docs/dashboard-prompts/)). **Steps 01–05 landed 2026-05-25** — scaffold + Tailwind v4 + shadcn/ui (31 primitives) + react-i18next (UZ canonical, RU/EN stubs) + react-router-dom v6 with `basename: /Devon/dashboard` + Zustand auth store + `RequireAuth` deep-link preserver + mobile-first split-pane login + AppShell with sidebar drawer (Sheet on mobile, persistent 240px column on lg+), sticky cream/85 backdrop-blur topbar with search + notifications + UserMenu, and a reusable `<PageHeader>` for every page. **`<main>` is full-width** — no max-w clamp — per the decision recorded in [`LESSONS.md`](./LESSONS.md) (Devon is data-dense admin; tables/kanban/audit benefit from horizontal room). Toaster position flips responsively via `useMediaQuery` (top-center on mobile, bottom-right on desktop). The 8 protected routes from master §11 now render inside the full Devon chrome. Demo creds (`admin@devon.uz` / `Demo2026!`) still hardcoded in [`useAuthStore.ts`](../dashboard/src/stores/useAuthStore.ts) until step 07 refactors to the mock-backend `users[]` table. Reset-demo wired in UserMenu (clears `devon.dashboard.*` localStorage keys + toast + reload). `npm run build` clean (94 KB CSS, 456 KB JS / 144 KB gzip). Outstanding from step 02: the shadcn `form` primitive's registry entry is silently skipped by the Nova preset — hand-add when a real form lands. Next session begins with `00-master.md` + `06-mock-backend.md` to wire the localStorage-backed mock API + zod schemas + seed data.

---

## Open questions / known gaps

- **Dashboard implementation in progress** — steps 01–05 are done; step 06 (mock backend over localStorage + zod schemas + seed data) is next, then sequentially through `15-final-qa.md`. Hand-add the shadcn `form` primitive when an actual form first appears (Nova preset's registry silently skips it). Out-of-scope items deliberately deferred per master §17: real backend, real PFX parsing, real E-IMZO plugin, real SMS/email OTP, automated tests.
